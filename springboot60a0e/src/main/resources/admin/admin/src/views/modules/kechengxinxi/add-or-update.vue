<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="课程课件" prop="kechengkejian">
          <file-upload
          tip="点击上传课程课件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.kechengkejian?ruleForm.kechengkejian:''"
          @change="kechengkejianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.kechengkejian" label="课程课件" prop="kechengkejian">
            <el-button type="text" size="small" @click="download(ruleForm.kechengkejian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengxiangqing = true;
          ...
          if(o=='kechengkejian'){
            this.ruleForm.kechengkejian = obj[o];
	    this.ro.kechengkejian = true;
            continue;
          }
          if(o=='jiaoshigonghao'){
	...
	this.ruleForm.kechengxiangqing = this.ruleForm.kechengxiangqing.replace(reg,'../../../springboot60a0e/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    kechengkejianUploadChange(fileUrls) {
	this.ruleForm.kechengkejian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
